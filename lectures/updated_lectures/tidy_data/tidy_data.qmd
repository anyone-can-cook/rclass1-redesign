...
-   A **cell** is the intersection of one column and one row (think cells in Microsoft Excel or Google sheets)

There are many alternative data structures to present the same underlying data.

<br> **Example**: 2 different ways to structure the same data (rows and columns are transposed)
...
<br> **Example**: Describing a dataset
...
<br> **Example**: Describing data semantics \[revisited\]
...
<br> First, Let's investigate whether the ID variable `unitid` uniquely identifies rows in data frame `ipeds_hc_temp`
...
<br> Second, Let's investigate whether the combination of `unitid` and `lstudy` uniquely identifies rows in data frame `ipeds_hc_temp`.
...
<br> **Tidy data** always follow these 3 interrelated rules:
...
<br> **Example**: Tidy vs. untidy data
...
1.  Does each **variable** have its own **column**? [No]{style="color: red;"}
    -   If not, how does the dataset violate this principle? [`cases` and `population` should be two different variables because they are different attributes, but in `table2` these two attributes are recorded in the column `type` and the associated value for each type is recorded in the column `count`.]{style="color: red;"}
    -   What *should* the variables be? [`country`, `year`, `cases`, `population`]{style="color: red;"}
2.  Does each **observation** have its own **row**? [No]{style="color: red;"}
    ...
    -   What does each row *actually* represent? [`country`-`year`-`type`]{style="color: red;"}
    -   What *should* each row represent? [`country`-`year`]{style="color: red;"}
3.  Does each **value** have its own **cell**? [Yes]{style="color: red;"}
    -   If not, how does the dataset violate this principle?
...
<br> Now that we have a better understanding of the differences between tidy and untidy data, let's practice reshaping our data. In the next two sections we are introducing the `pivot_longer()` and `pivot_wider()` functions to reshape our data.
...
<br> **Example**: Tidying `table4b` (reshaping wide to long)
...
<br> The solution to this problem is to transform the untidy columns (which represent variable values) into rows. Thus, we want to transform `table4b` into something that looks like this:
...
<br> This can be achieved using `pivot_longer()`:
...
<summary>**Example**: Choosing which column not to pivot</summary>
...
<summary>**Example**: Renaming pivoted columns</summary>
...
<br> If we pivot the table as is, we can see the `tot_` prefixes appearing in the `year` column:
...
<br> To fix this, we can specify the `names_prefix` argument in `pivot_longer()` to match and remove the part of the names that we don't want to keep:
...
<summary>**Example**: Pivoting columns by name pattern</summary>
...
<br><br> How we will use these arguments to reshape `nces_table2`:
...
<br> Note the difference if we used `.value` as the second element in `names_to`:
...
<br> **Practical example** of using the `pivot_longer()` function [in the Appendix](#appendix)

## Reshaping long to wide: `pivot_wider()`

<br> **The `pivot_wider()` function**:
...
<br> **Example**: Tidying `table2` (reshaping long to wide)
...
<br> This sort of data structure is very common "in the wild" (e.g., in data that you download), and it is up to you to tidy it before analyses.
...
<br> This can be achieved using `pivot_wider()`:
...
<summary>**Example**: Pivoting with multiple value variables</summary>
...
<br> The goal is to turn the values in `levels` into columns - but which of the enrollment variables should be used as the values for the new columns? The solution is to create columns for each of the enrollment variables:
...
<br> **Practical example** of using the `pivot_wider()` function [in the Appendix](#appendix)
...
<br> **Example**: Identifying explicit and implicit missing values
...
<br> **Practical example** of identifying explicit and implicit missing values [in the Appendix](#appendix)
...
<br> **Example**: Making implicit missing values explicit
...
<br> We can achieve this by using the `complete()` function:
...
<br> Should you make **implicit** missing values **explicit**?
...
<br> **Practical example** of using the `complete()` function [in the Appendix](#appendix)
...
<br> When we reshape it from long to wide using `pivot_wider()`, notice that both implicit and explicit missing values appear as `NA`. In other words, implicit missing values are automatically turned into explicit missing values. Therefore, there is no need to use `complete()` prior to pivoting.
...
<summary>**Solution to student exercise:**</summary>
...
<br> The Integrated Postsecondary Education Data System (IPEDS) collects data on colleges and universities
...
<summary>**Solution to student task:**</summary>
