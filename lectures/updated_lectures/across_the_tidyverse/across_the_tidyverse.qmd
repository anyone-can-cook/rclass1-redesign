...
__Processing across variables vs. processing across observations__
...
**Creating graphs and tables of descripive stats usually require calculations across obs**
...
__Processing across obs (focus of today's lecture)__
...
__Strategy for teaching processing across obs__
...
__Combining `summarize()` and `group_by`__
...
__Isolate first/last/nth observation in a group__
...
-   For U. Pittsburgh (`univ_id = 215293`) events at public and private high schools, caclulate the **mean** value of `med_inc` and `pct_white_zip` for each combination of `event_type` and `event_inst`
...
-   For U. Pittsburgh (`univ_id = 215293`) events at public and private high schools, caclulate **mean** and **standard deviation** of `med_inc` and `pct_white_zip` for each combination of `event_type` and `event_inst`
