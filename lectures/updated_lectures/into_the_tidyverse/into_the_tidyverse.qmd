---
title: "Into the tidyverse: dplyr, select, filter, and pipes"
author: ""
date: ""
format: 
  html:
...
## Data for sections on `select()`, `filter()`, and `arrange()` functions
...
## Data for sections on pipes `%>%` and `mutate()` function
...
-   `rename(obj_name, new_name = old_name,...)` \medskip
...
Variable names do not change permanently unless we combine rename with assignment \medskip
...
## `count()` function with pipes `%>%`

\medskip Recall `count()` function from `dplyr` package counts the number of obs by group
...
<summary><strong>Solutions pipes `%>%`</strong></summary>
...
We will load the data frame __`df_school_all`__. This data frame is based on data from the recruiting project. It contains one observation per US high school and variables identifying the number of visits by particular universities. 
...
<!-- BREAK DOWN THE EXAMPLE HERE, INTRODUCE MUTATE USING A SIMPLE EXAMPLE -->
