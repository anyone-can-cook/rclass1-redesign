...
**Example**: Scatterplot of the relationship between parent income (`parent_income`) and students' earnings in 2011 (`f3ern2011`), using the `els` dataset

-   Color of points determined by type race/ethnicity (`f1racae`):

```{r}
els_v2 %>%
  filter(parent_income < 150000 & f3ern2011 > 0 & f3ern2011 < 100000) %>%
  ggplot(mapping = aes(x = parent_income, y = f3ern2011, color = f1race)) +
  geom_point(position = "jitter")
```
...
els_v2 %>%
  filter(parent_income < 150000 & f3ern2011 > 0 & f3ern2011 < 100000) %>%
  ggplot(mapping = aes(x = parent_income, y = f3ern2011)) +
...
#check variable f2evratt
typeof(els_v2$f2evratt)
class(els_v2$f2evratt)
str(els_v2$f2evratt)
attributes(els_v2$f2evratt)$levels

#new factor var
els_v2$attend_coll_fac <- factor(els_v2$f2evratt,
...
#left off here, figure out better variables to use
els_v2 %>%
  filter(parent_income < 150000 & f3ern2011 > 0 & f3ern2011 < 100000) %>%
  ggplot(mapping = aes(x = parent_income, y = f3ern2011)) +
  geom_point(position = "jitter") +
  facet_grid(rows = vars(h_degree))
```
