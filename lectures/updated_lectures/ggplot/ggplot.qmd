...
-     Let's recode some variables we will use later to make them easier to work with.
```{r}
...
-     For example, below we create the object `diamond_colors` and assign the values of the variable `color` to hex color codes. Next, we add the new object `diamond_colors` to the values argument from the `scale_color_manual` function. 
```{r}
...
diamonds %>% 
  ...
  xlab('Carat') + ylab('Price') +
  scale_y_continuous(labels = label_number(prefix = '$', suffix = 'K', scale = 1e-3, accuracy = 1)) +
  scale_color_manual(values = diamond_colors,
                     name = 'Diamond color')
```

### Grayscale palette for journal manuscripts

Many journals require grayscale graphs or figures. You can use the `scale_colour_grey()` function for continuous or the `scale_fill_grey()` function for discrete color scales. 
