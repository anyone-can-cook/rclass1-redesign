...
-   Install "tidyverse": `install.packages("ggplot2")`
...
els <- els %>% 
  mutate(
    ...
    totloan_v2 = as.numeric(format(els$f3totloan, scientific = FALSE))
  )
...
-   Layered grammar of graphics [@RN4561]
    ...
    -   The layered grammar of graphics is a "formal system for building plots... based on the insight that you can uniquely describe *any* plot as a combination of" seven paramaters [@RN4564, chapter 3]
-   Aesthetics
...
-   Note we filtered to less than \$50k borrowed in loans and less than \$100K in earnings due to the large number of observations. 
...
els %>% select(totloan_v2) %>% summary()
els %>%count(totloan_v2)

#mean roughly $63k, max $250k, median $32k, lets filter for $150k or less
els %>% select(parent_income) %>% summary()
els %>% count(parent_income)

#know credits earned
attributes(els$f3tzpostern)
els %>% select(f3tzpostern) %>% summary()

#earnings in 2011, mean $29k, median $25k
els %>% filter(f3ern2011 > 0) %>% select(f3ern2011) %>% summary()
els %>% count(f3ern2011)

#highest known degree attained as of June 2013
attributes(els$f3tzhighdeg)
els %>% select(f3tzhighdeg) %>% summary()
```
...
els %>%
  filter(totloan_v2 < 50000 & f3ern2011 > 0 & f3ern2011 < 100000) %>%
  ggplot(aes(x = totloan_v2, y = f3ern2011)) +
  geom_point()
...
els %>%
  ...
  ggplot(mapping = aes(x = h_degree, y = totloan_v2)) + 
  geom_boxplot() +
...
els %>%
  filter(totloan_v2 < 50000 & f3ern2011 > 0 & f3ern2011 < 100000) %>%
  ggplot() + 
  geom_point(mapping = aes(x = totloan_v2, y = f3ern2011), position = "jitter") +
  facet_wrap(~f1race, nrow = 3)
```
...
els_parphd %>% count(bystexp)
els_parphd %>% count(f3evratt)

els_parphd <- els_parphd %>%
  mutate(ever_college = recode(f3evratt, "Survey component legitimate skip/NA" = "Skip",
...
**Example**: Scatterplot of the relationship between parent income (`parent_income`) and student earnings in 2011 (`earn2011`), using the `els` dataset

-   Color of points determined by type race/ethnicity (`f1race`):

```{r}
els_parphd %>%
  #filter(f3ern2011 > 0 & f3ern2011 < 100000) %>%
  ggplot(mapping = aes(x = parent_income, y = earn2011, color = f1race)) +
  geom_point(position = "jitter")
```
...
els_parphd %>%
  ...
  geom_point(position = "jitter", mapping = aes(color = f1race))
```
...
**Student Task**: Using the `els_v2` dataset, create a bar chart with the variable "ever attended postsecondary education" (`f3evratt`) as x-axis and number of students as y-axis
...
#glimpse(els_v2)
els_v2 %>% select(f3evratt) %>% var_label()

#ever attended
els_v2 %>% count(f3evratt)
```
...
```{r}
ggplot(data = els_v2, aes(x = f3evratt)) +
  geom_bar()
```

-   Additionally, we can use pipes to filter values of `f3evratt` before plotting:

```{r}

#check variable f3evratt
typeof(els_v2$f3evratt)
class(els_v2$f3evratt)
str(els_v2$f3evratt)
attributes(els_v2$f3evratt)$levels

#new factor var
els_v2$attend_coll_fac <- factor(els_v2$f3evratt,
                                 labels = c("Survey component legitimate skip/NA", "Nonrespondent", "Item legitimate skip/NA", "No", "Yes"))

#check variable
els_v2 %>%
  count(attend_coll_fac)

els_v2 %>%
  count(f3evratt)

typeof(els_v2$attend_coll_fac)
class(els_v2$attend_coll_fac)

#els_v2 %>% filter(f3evratt %in% c('No','Yes')) %>% ggplot(aes(x = f3evratt)) +
#  geom_bar()

els_v2 %>% filter(attend_coll_fac %in% c('No','Yes')) %>% ggplot(aes(x = attend_coll_fac)) +
 geom_bar()
```
...
#  filter(f3evratt %in% c('No','Yes')) %>% 
  # use count() to create summary statistics object
#  count(f3evratt) %>%
  # plot summary statistic object
#  ggplot(aes(x=f3evratt, y=n)) + geom_col()

els_v2 %>% 
  # filter to remove missing values
  filter(attend_coll_fac %in% c('No','Yes')) %>% 
  # use count() to create summary statistics object
  count(attend_coll_fac) %>%
  # plot summary statistic object
  ggplot(aes(x=attend_coll_fac, y=n)) + geom_col()
...
# Facet into rows
els_parphd %>%
  filter(ever_college %in% c("No college", "Some college") & parent_income < 150000 & earn2011 < 100000) %>%
  ggplot(mapping = aes(x = parent_income, y = earn2011)) +
  geom_point(position = "jitter") +
...
```{r}
els_parphd %>%
  filter(ever_college %in% c("N college", "Some college") & parent_income < 150000 & earn2011 < 100000) %>%
  ggplot(mapping = aes(x = parent_income, y = earn2011)) +
  geom_point() +
...
els_parphd %>%
  filter(ever_college %in% c("No college", "Some college") 
         & ed_aspiration %in% c("Don't know","No HS diploma", "HS diploma or GED", "2-year", "Attend college, no Bachelors ", "Bachelor's", "Master's", "Doctorate") & parent_income < 150000 & earn2011 < 100000) %>%
  ggplot(mapping = aes(x = parent_income, y = earn2011)) +
  geom_point(position = "jitter") +
...
els_parphd %>%
  ...
  ggplot(mapping = aes(x = parent_income, y = earn2011)) +
  geom_point(position = "jitter") +
  facet_wrap(ever_college ~ ed_aspiration, nrow = 5)
```
...
-   from [brilliance.com](https://www.brilliance.com/education/diamonds/depth-table): "Depth percentage is the diamond’s depth \[height\] divided by the width of the diamond. This percentage dictates the overall proportions of the diamond, which in turn directly impact how light reflects off the facets in the stone.
