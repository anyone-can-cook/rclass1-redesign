...
<summary>**Investigating student level `nls_stu` dataframe**</summary>
...
<summary>**Investigating student level `nls_stu_pets` dataframe**</summary>
...
<summary>**Investigating student-transcript level `nls_tran` dataframe**</summary>
...
<summary>**Investigating student-transcript-term level `nls_term` dataframe**</summary>
...
<br> There are two types of keys: **primary keys** and **foreign keys**
...
<br> **Example**: `id` is the primary key in the `nls_stu` table
...
<br> **Example**: `id` and `transnum` make up the primary key in the `nls_tran` table
...
<br> **Example**: With respect to datasets `nls_stu` and `nls_tran`
...
<summary>**Ozan's way of thinking about foreign key**</summary>
...
*Note: Credit for examples and images belong to [Wickham](https://r4ds.had.co.nz/relational-data.html#mutating-joins)*
...
<br> There are four types of joins between tables `x` and `y`:
...
<br> We will join tables `x` and `y` using the `join()` command from `dplyr` package. `join()` is a general command, which has more specific commands for each type of join:
...
**Inner joins** keep all observations that appear in both table `x` and table `y` (i.e., keep observations <code style="background-color:lightgreen">1</code> and <code style="background-color:lavender">2</code>):

![](https://raw.githubusercontent.com/anyone-can-cook/rclass1/master/assets/images/join-inner.png)

<br> **The `inner_join()` function**:
...
<br> **Example**: Using `inner_join()` to join `x` and `y`
...
<br> This is an example of a **one-to-one** relationship because `key` uniquely identifies observations in both table `x` and `y`. In other words, `key` can be the *primary key* for both `x` and `y`, as well as *foreign key* when we think about the relationship between the tables. <!-- https://stackoverflow.com/a/30517805/6373540 -->

<br>

<details>

<summary>**Example**: One-to-one inner join between `nls_stu` and `nls_stu_pets`</summary>
...
<summary>**Summary of the types of relationships**</summary>
...
-   **one-to-many** join: key variable uniquely identifies observatiosn in table `x` and does not uniquely identify observations in table `y`
...
<summary>**Example**: One-to-many inner join between `nls_stu` and `nls_trans`</summary>
...
<br> **Example**: Joining `nls_tran` and `nls_term` by multiple variables
...
<br> To perform each of the 3 types of **outer joins**:
...
<summary>**Example**: Left joins between `nls_stu`, `nls_stu_pets`, and `nls_tran`</summary>
...
<summary>**Example**: Using `anti_join()` to diagnose mismatches in mutating joins</summary>
...
<br> As seen, **18855** of the **24253** student transcripts (rows) in `nls_tran` have a match in `nls_term`, leaving a remaining **5398** student transcripts that did not match to any term level info. We can use `anti_join()` to investigate these unmatched rows to see why these transcripts do not have any term level data.
...
<br> **Example**: Using `bind_rows()` to bind `time1` and `time2`
...
<br> Use `bind_rows()` to stack the dataframes on top of one another:
...
<summary>**Example**: Creating longitudinal dataset from IPEDS data</summary>
...
<summary>**Example**: Appending datasets where not all columns match</summary>
...
<summary>**Overcoming join problems before they arise**</summary>
...
3.  Before joining, you also want to make sure the key variable in one table and key variable in another table are the same type (both numeric or both string, etc.)
    ...
    -   If you try to join with key variables of different type, you will get this error message: [Can't join on 'key' x 'key' because of incompatible types (character / numeric)]{style="color:red"}\
...
<summary>**Overcoming join problems when they do arise**</summary>
...
*Source: [R for Data Science](https://r4ds.had.co.nz/relational-data.html#mutating-joins) by Wickham*
