...
**Example**: Create a tibble to investigate atomic vectors

```{r}
#create a tibble with columns or variables of different atomic vectors 
v <- tibble(l = c(TRUE, FALSE, FALSE, TRUE),
            n = c(1, 3, 5, 7),
            i = c(1L, 3L, 5L, 7L),
            c = c("a", "b", "c", "d"))
...
<summary>**Example**: Using `str_length()` on a character vector</summary>
...
__Creating date/time objects by parsing input__
...
__Creating date/time objects from individual components__
...
__Time spans using `lubridate`__
...
__Escape special characters using Twitter data__
